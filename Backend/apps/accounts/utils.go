...
func Signup(user RegisterInput) (int, string, map[string]interface{}) {
	...
	tx, err := fabric.Smartcontract.SubmitTransaction("RegisterProfile", newuser.FirstName, newuser.LastName, newuser.Email, newuser.Company.Name)
	if err != nil {
		fmt.Println("Error creating block in ledger", err.Error())
	}
	fmt.Println(string(tx))

	if err := db.DB.Model(&newuser).Where("email = ?", newuser.Email).Update("transaction_hash", string(tx)).Error; err != nil {

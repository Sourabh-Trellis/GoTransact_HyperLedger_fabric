...
func SendMail(user accountmodels.User, request transactionmodels.TransactionRequest) {
	...
	baseURL := "http://localhost:8081/api/confirm-payment" // Replace with your actual domain and endpoint
	params := url.Values{}
...
func PostPayment(Postpaymentinput PostPaymentInput, user accountmodels.User) (int, string, map[string]interface{}) {
	...
	Tx, err := fabric.Smartcontract.SubmitTransaction("TransactionDetails", string(TransactionRequest.ID), string(TransactionRequest.Status), fmt.Sprintf("%.2f", TransactionRequest.Amount))
	if err != nil {
		fmt.Println("error committing transaction in leger,", err.Error())
	}

	if err := db.DB.Model(&TransactionHistory).Where("transaction_id = ?", TransactionRequest.ID).Update("transaction_hash", string(Tx)).Error; err != nil {
		fmt.Println("error updating transaction hash")
	}
...
func ConfirmPayment(transactionIdStr, statusStr string) (int, string, map[string]interface{}) {
	...
	if strings.EqualFold(statusStr, "true") {
		...
		if err := db.DB.Model(&trasactionHistory).Where("transaction_id = ?", transactionRequest.ID).Update("transaction_hash", string(Tx)).Error; err != nil {
			fmt.Println("error updating transaction hash")
		}
